...
type ExpandedSchedule struct {
	...
	WeeksOfYearMask            uint64 // Support weeks 1-53 (bits 1-53)
	Years                      []int
	DayOfMonth, DayOfWeek      string
	WeekOfYear                 string // Original week expression for special patterns
...
func (e *Engine) findPrev(fromTime time.Time, s *ExpandedSchedule) (time.Time, error) {
	...
	for {
		...
		if !e.isWeekOfYearMatch(t, s) {
			...
			if newT.Equal(t) || newT.After(t) {
				return time.Time{}, fmt.Errorf("no valid week found")
			}
			t = newT
			continue
		}
...
func (e *Engine) getExpandedSchedule(schedule Schedule) (*ExpandedSchedule, error) {
	...
	if schedule.WeekOfYear != nil {
		WOY = *schedule.WeekOfYear
	}
	...
	// Parse week of year (1-53)
	var woyMask uint64
	if woyMask, err = expandPart(WOY, 1, 53); err != nil {
		return nil, fmt.Errorf("invalid week of year expression '%s': %w", WOY, err)
	}
	...
	exp.DayOfMonth, exp.DayOfWeek, exp.WeekOfYear = D, dow, WOY
...
func (e *Engine) buildCacheKey(schedule Schedule) string {
	...
	if schedule.WeekOfYear != nil {
		sb.WriteString(*schedule.WeekOfYear)
	} else {
		sb.WriteByte('*')
	}
	sb.WriteByte('|')
...
// isWeekOfYearMatch checks if the given time matches the week of year mask
func (e *Engine) isWeekOfYearMatch(t time.Time, s *ExpandedSchedule) bool {
	...
	// Handle year boundary cases: if ISO week belongs to different year,
	// we need to adjust based on the current calendar year
	if year != t.Year() {
		// Week belongs to previous/next year
		if t.Month() == time.January && week > 50 {
			// This is week 52/53 of previous year, but we're in January
			// In our context, this should be treated as week 0 or not matching
			return false
		} else if t.Month() == time.December && week < 10 {
			// This is week 1 of next year, but we're in December
			// In our context, this should be treated as week 53+ or not matching
			return false
		}
	}

	// Check if the week matches the mask (bits 1-53)
	if week >= 1 && week <= 53 {
		return (s.WeeksOfYearMask & (1 << uint(week))) != 0
	}
	...
	if len(pattern) >= 2 && pattern[len(pattern)-1] == 'L' {
		...
		if day, err := strconv.Atoi(dayStr); err == nil && day >= 0 && day <= 6 {
			...
			if currentWeekday == targetWeekday {
				...
				lastDayOfMonth := time.Date(t.Year(), t.Month(), 0, 0, 0, 0, 0, t.Location()).Day()
				return currentDay > lastDayOfMonth-7
			}
		}
		return false
