...
class ValidationPatterns {
  ...
  static readonly basicCronPattern = /^(\*|[\d\-,\/]+)\s+(\*|[\d\-,\/]+)\s+(\*|[\d\-,\/]+)\s+(\*|[\d\-,\/\?L#]+)\s+(\*|[\d\-,\/]+)\s+(\*|[\d\-,\/\?L#]+)(?:\s+(\*|[\d\-,\/]+))?$/;
  
  static readonly jcronPattern = /^(\*|[\d\-,\/]+)\s+(\*|[\d\-,\/]+)\s+(\*|[\d\-,\/]+)\s+(\*|[\d\-,\/\?L#]+)\s+(\*|[\d\-,\/]+)\s+(\*|[\d\-,\/\?L#]+)(?:\s+(\*|[\d\-,\/]+))?(?:\s+(?:TZ:|WOY:|EOD:).+)*$/;
  
  // Fast field validation patterns
  static readonly validSecond = /^(\*|[0-5]?\d(?:-[0-5]?\d)?(?:,[0-5]?\d(?:-[0-5]?\d)?)*(?:\/\d+)?)$/;
  static readonly validMinute = /^(\*|[0-5]?\d(?:-[0-5]?\d)?(?:,[0-5]?\d(?:-[0-5]?\d)?)*(?:\/\d+)?)$/;
  static readonly validHour = /^(\*|[01]?\d|2[0-3](?:-(?:[01]?\d|2[0-3]))?(?:,(?:[01]?\d|2[0-3])(?:-(?:[01]?\d|2[0-3]))?)*(?:\/\d+)?)$/;
  static readonly validDay = /^(\*|\?|L|[1-9]|[12]\d|3[01](?:-(?:[1-9]|[12]\d|3[01]))?(?:,(?:[1-9]|[12]\d|3[01])(?:-(?:[1-9]|[12]\d|3[01]))?)*(?:\/\d+)?|[0-7]#[1-5]|[0-7]L)$/;
  static readonly validMonth = /^(\*|[1-9]|1[0-2](?:-(?:[1-9]|1[0-2]))?(?:,(?:[1-9]|1[0-2])(?:-(?:[1-9]|1[0-2]))?)*(?:\/\d+)?)$/;
  // Allow multiple #N and L patterns: "1#1,3L,5L" or "1#2,2#4"
  static readonly validDow = /^(\*|\?|(?:[0-7](?:#[1-5]|L)?|[0-7](?:-[0-7])?(?:\/\d+)?)(?:,(?:[0-7](?:#[1-5]|L)?|[0-7](?:-[0-7])?(?:\/\d+)?))*)$/;
  static readonly validYear = /^(\*|19\d\d|20\d\d|21\d\d(?:-(?:19\d\d|20\d\d|21\d\d))?(?:,(?:19\d\d|20\d\d|21\d\d)(?:-(?:19\d\d|20\d\d|21\d\d))?)*(?:\/\d+)?)$/;
}
...
function isJCronExpression(expression: string): boolean {
  return expression.includes('TZ:') || expression.includes('WOY:') || expression.includes('EOD:');
}
...
function validateScheduleFields(schedule: Schedule): boolean {
  ...
  if (schedule.s && !ValidationPatterns.validSecond.test(schedule.s)) return false;
  if (schedule.m && !ValidationPatterns.validMinute.test(schedule.m)) return false;
  if (schedule.h && !ValidationPatterns.validHour.test(schedule.h)) return false;
  if (schedule.D && !ValidationPatterns.validDay.test(schedule.D)) return false;
  if (schedule.M && !ValidationPatterns.validMonth.test(schedule.M)) return false;
  if (schedule.dow && !ValidationPatterns.validDow.test(schedule.dow)) return false;
  if (schedule.Y && !ValidationPatterns.validYear.test(schedule.Y)) return false;
  
  return true;
}

// Optimize validateCron function
export function validateCronOptimized(cronExpression: string | Schedule): { valid: boolean; errors: string[] } {
  if (typeof cronExpression !== 'string') {
...
export function getPatternOptimized(cronExpression: string): "daily" | "weekly" | "monthly" | "yearly" | "custom" {
